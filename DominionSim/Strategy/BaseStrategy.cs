...
namespace DominionSim.Strategy
{
    abstract class BaseStrategy : IStrategy
    {
        ...
        public virtual IEnumerable<Card> ChooseCardsToTrash(PlayerFacade p, int min, int max, CardType type, Supply s)
        {
        ...
        public virtual IEnumerable<Card> ChooseOpponentCardsToTrash(PlayerFacade p, int min, int max, string opponent, IEnumerable<Card> cards)
        {
        ...
        /// <param name="s"></param>
        /// <returns></returns>
        public virtual IEnumerable<Card> ChooseCardsToDiscard(PlayerFacade p, int min, int max, CardType type, Supply s)
        {
            var orderedCards = p.GetHand().Where( c => (CardList.Cards[c].Type & type) != 0 )
                                          .OrderBy( c => (CardList.Cards[c].Cost) )
                                          .OrderByDescending(c => (CardList.Cards[c].Type & (CardType.Curse | CardType.Victory)) != 0);
        ...
        public Card ChooseCardToGainFromSupply(PlayerFacade p, int minCost, int maxCost, CardType type, Supply s)
        {
        ...
        public virtual IEnumerable<Card> ChooseOpponentCardsToGain(PlayerFacade p, int min, int max, string opponent, IEnumerable<Card> cards)
        {
            // Always gain any non-copper treasure, nothing else
            return cards.Where(c => c == Card.Silver || c == Card.Gold).Take(max);
        }
        ...
        /// <returns></returns>
        protected bool CanAfford(PlayerFacade p, Card card)
        {
            return (p.GetMoneys() >= CardList.Cards[card].Cost);
        }
        ...
        public IEnumerable<Card> ChooseReactionsToAttack(PlayerFacade victimPlayerFacade, Supply supply, string attackerName, Card card)
        {
            // Naive implementation - just react with everything
            return victimPlayerFacade.GetHand().Where(c => ((CardList.Cards[c].Type & CardType.Reaction) != 0));
        }
